...
`Dockerfile` (repo root) builds one image containing the Django backend, the Celery worker, Celery beat, and the built Next.js frontend, plus `nginx` and Node 20 (installed via NodeSource) and `honcho` (a Foreman-style process runner). `Procfile` (repo root) lists what honcho supervises: `proxy` (nginx), `web` (gunicorn on `127.0.0.1:8000`), `frontend` (`next start` on `127.0.0.1:3000`), `worker` (Celery worker), `beat` (Celery beat). honcho's default behavior is strict: if any one process exits, it stops all the others and exits too, so a silent Celery beat crash can't go unnoticed — Render's restart policy then restarts the whole container.
