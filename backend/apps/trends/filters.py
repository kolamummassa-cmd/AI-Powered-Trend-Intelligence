...
class TrendFilter(django_filters.FilterSet):
    ...
    stage = django_filters.ChoiceFilter(choices=TrendStage.choices, method="filter_stage")
    since = django_filters.IsoDateTimeFilter(field_name="first_detected_at", lookup_expr="gte")
    min_trend_score = django_filters.NumberFilter(method="filter_min_trend_score")
    min_opportunity_score = django_filters.NumberFilter(method="filter_min_opportunity_score")
    ...
    def filter_high_priority(self, queryset, name, value):
        ...
        return queryset.filter(
            analyses__created_by=self.request.user,
    ...
    def filter_stage(self, queryset, name, value):
        return queryset.filter(
            analyses__created_by=self.request.user, analyses__trend_stage=value
        ).distinct()

    def filter_min_trend_score(self, queryset, name, value):
        return queryset.filter(
            analyses__created_by=self.request.user, analyses__trend_score__gte=value
        ).distinct()

    def filter_min_opportunity_score(self, queryset, name, value):
        return queryset.filter(
            analyses__created_by=self.request.user, analyses__opportunity_score__gte=value
        ).distinct()
    ...
    def filter_audience(self, queryset, name, value):
        ...
        return queryset.filter(
            analyses__created_by=self.request.user,
            **{f"analyses__{score_field}__gte": AUDIENCE_RELEVANCE_THRESHOLD},
        ).distinct()
