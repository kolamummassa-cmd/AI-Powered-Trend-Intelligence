...
import { BrainCircuitIcon, RadarIcon, SearchIcon, SparklesIcon } from "lucide-react";
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
        <div className="relative flex-1"><SearchIcon className="pointer-events-none absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" /><Input placeholder="Search trends..." value={search} onChange={(e) => { setSearch(e.target.value); resetPage(); }} className="pl-9" /></div>
        <select value={status} onChange={(e) => { setStatus(e.target.value as typeof status); resetPage(); }} className="h-10 w-full rounded-md border border-input bg-transparent px-3 text-sm sm:w-auto">{STATUS_OPTIONS.map((option) => <option key={option} value={option} className="bg-card">{option === "all" ? "All statuses" : option}</option>)}</select>
        <select value={audience} onChange={(e) => { setAudience(e.target.value as typeof audience); resetPage(); }} className="h-10 w-full rounded-md border border-input bg-transparent px-3 text-sm sm:w-auto">{AUDIENCE_OPTIONS.map((option) => <option key={option} value={option} className="bg-card">{option === "all" ? "All audiences" : AUDIENCE_LABELS[option]}</option>)}</select>
        <select value={stage} onChange={(e) => { setStage(e.target.value as typeof stage); resetPage(); }} className="h-10 w-full rounded-md border border-input bg-transparent px-3 text-sm sm:w-auto">{STAGE_OPTIONS.map((option) => <option key={option} value={option} className="bg-card">{option === "all" ? "Any stage" : TREND_STAGE_LABELS[option]}</option>)}</select>
        <label className="flex h-10 w-full items-center gap-2 rounded-md border border-input px-3 text-sm sm:w-auto"><input type="checkbox" checked={highPriorityOnly} onChange={(e) => { setHighPriorityOnly(e.target.checked); resetPage(); }} />High priority</label>
        <label className="flex h-10 w-full items-center gap-2 rounded-md border border-input px-3 text-sm sm:w-auto"><input type="checkbox" checked={kuzanaOnly} onChange={(e) => { setKuzanaOnly(e.target.checked); resetPage(); }} />Kuzana relevant</label>
      </div>
      ...
      {data && data.results.length === 0 && <FeatureEmptyState badge="Trend engine" title="No matching trends yet." description="Try broadening your search or filters. New signals are added automatically as they are verified." benefits={[{ icon: RadarIcon, title: "Collect", description: "Sources are scanned continuously." }, { icon: BrainCircuitIcon, title: "Analyze", description: "Every useful signal is scored." }, { icon: SparklesIcon, title: "Act", description: "Turn a relevant trend into content." }]} />}
