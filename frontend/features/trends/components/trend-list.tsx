  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-col gap-3 rounded-2xl border border-border bg-card/80 p-3 sm:flex-row sm:items-center">
        <div className="relative flex-1"><SearchIcon className="pointer-events-none absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" /><Input placeholder="Search trends..." value={search} onChange={(e) => { setSearch(e.target.value); resetPage(); }} className="bg-background pl-9" /></div>
        <select value={status} onChange={(e) => { setStatus(e.target.value as typeof status); resetPage(); }} className="h-10 w-full rounded-md border border-border bg-background px-3 text-sm sm:w-auto">{STATUS_OPTIONS.map((option) => <option key={option} value={option} className="bg-card">{option === "all" ? "All statuses" : option}</option>)}</select>
        <select value={audience} onChange={(e) => { setAudience(e.target.value as typeof audience); resetPage(); }} className="h-10 w-full rounded-md border border-border bg-background px-3 text-sm sm:w-auto">{AUDIENCE_OPTIONS.map((option) => <option key={option} value={option} className="bg-card">{option === "all" ? "All audiences" : AUDIENCE_LABELS[option]}</option>)}</select>
        <select value={stage} onChange={(e) => { setStage(e.target.value as typeof stage); resetPage(); }} className="h-10 w-full rounded-md border border-border bg-background px-3 text-sm sm:w-auto">{STAGE_OPTIONS.map((option) => <option key={option} value={option} className="bg-card">{option === "all" ? "Any stage" : TREND_STAGE_LABELS[option]}</option>)}</select>
        <label className="flex h-10 w-full items-center gap-2 rounded-md border border-border bg-background px-3 text-sm sm:w-auto"><input type="checkbox" checked={highPriorityOnly} onChange={(e) => { setHighPriorityOnly(e.target.checked); resetPage(); }} />High priority</label>
      </div>
      ...
      {isError && <div className="rounded-2xl border border-destructive/20 bg-destructive/5 p-5"><p className="font-medium">We couldn&apos;t load trends right now.</p><p className="mt-1 text-sm text-muted-foreground">Your filters are still saved. Please try again.</p><Button className="mt-4" size="sm" variant="outline" onClick={() => refetch()}>Retry</Button></div>}
